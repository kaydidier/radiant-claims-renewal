...
if(@$_GET['action']=='delete') {
	switch (@$_GET['type']) {
		case 'car':
			$prid=$_GET['prid'];
			if (mysql_query("DELETE FROM cars where cars.car_id='$prid'")or die(mysql_error())) {
				mysql_query("DELETE FROM insured where propertyId='$prid'");
			...
			case 'house':
				$prid=$_GET['prid'];
			if (mysql_query("DELETE FROM houses where houses.house_id='$prid'")or die(mysql_error())) {
				mysql_query("DELETE FROM insured where propertyId='$prid'");
				header("location=empclient.php?action=viewclient&clid={$_GET['clid']}");
				}
			break;
...
<?php
 	...
 	if (@$_GET['action']=='viewclient') {
 		?>
 		<?php
		//$query=mysql_query("SELECT * from insured,clients,province,district where clients.id_client=insured.id_client and province.provinceId=clients.province and district.districtId=clients.district and clients.id_client={$_GET['clid']}");
		$query=mysql_query("SELECT * from insured,clients,district,province where insured.id_client=clients.id_client and district.districtId=clients.district and province.provinceId=clients.province and clients.id_client={$_GET['clid']}");


	?>
	<div id="addpro"><a href="initsession.php?clid=<?php echo $_GET['clid'];?>"><p>Add property>></p></a></div>
	<table>
	<?php
	$display=4;
	$cols=0;
			...
			<td>
				<?php
					switch ($queryData['type']) { 
						case 'house':
							$queryProperty=mysql_query("SELECT * FROM houses where house_id={$queryData['propertyId']}") or die(mysql_error());
							$queryPropertyData=mysql_fetch_array($queryProperty);
							?>
							<img src="images/houses/ <?php echo $queryPropertyData['photo'];?>" width="100" height="100"></td>
							<tr>
						...
						case 'car':
							$queryProperty=mysql_query("SELECT * FROM cars where car_id={$queryData['propertyId']}")or die(mysql_error());
							$queryPropertyData=mysql_fetch_array($queryProperty);
							?>
